...
const SecretPage = () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        if (data.sole) {
          ...
          message =
            "Welcome to your secret webpage. Keep your credentials private.";
        }

        typeText(message, setPostLoginMsg, () => setShowGallery(true));
        fetchFiles(); // fetch appropriate files
      } else {
        const invalidMsg =
  ...
  const fetchFiles = async (type) => {
    try {
      ...
      if (soleUser) {
        fetchType = "private"; // sole user can only see private
      ...
      const res = await fetch(
        `${backendUrl}/api/list-files?type=${fetchType}&username=${username}&password=${password}`
      );
      const data = await res.json();
            ...
            <>
                ...
                <div className="header-buttons">
                  {isAdmin && (
                    <>
                      ...
                      <button className="list-users-btn" onClick={handleToggleUsers}>
                        {loadingUsers
                          ? "Loading..."
                          : showUsersList
                          ? "Hide Users"
              ...
              {showCreateForm && isAdmin && (
                <form className="create-user-form" onSubmit={handleCreateUser}>
                  ...
                  <button type="button" className="cancel" onClick={toggleCreateForm}>
                    Cancel
                  </button>
                </form>
              )}
              ...
              <div className="folder-buttons">
                {isAdmin && (
                  <>
                    <button
                      onClick={() => fetchFiles("public")}
                      className={activeFolder === "public" ? "active" : ""}
                    >
                      Public
                    </button>
                    <button
                      onClick={() => fetchFiles("private")}
                ...
                {soleUser && <button className="active">Private Only 💖</button>}
                {!isAdmin && !soleUser && (
